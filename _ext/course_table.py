...
#         newrows.append(["Training module name", "Module description", "Estimated hours", "Discipline", "Resource Link", ])
#         if has_local:
...
# Load all course data from YAML files
COURSES = {}
BASEDIR = os.path.dirname(os.path.dirname(__file__))
for file_ in glob.glob(os.path.join(BASEDIR, 'courses/*.yaml')):
...
class CourseTable(YamlTable):
    def transform_yaml(self, data):
        ...
        for row in data:
            course_name = row.get('id', '')
            self.ids.append(course_name)
            ...
            new_row = [
                ...
                row.get('hours', ''),
                row.get('discipline', ''),
                '<%s>' % row['link'] if row.get('link') else ''
            ]
...
class Course(Directive):
    ...
    def run(self):
        ...
        course_data = COURSES.get(id_)
        if not course_data:
            error = self.state_machine.reporter.error(
                f"Course ID '{id_}' not found in course data.",
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [error]

        # Build key-value info table
        rows = []
        for key, label in [
            ('desc', 'Description'),
            ...
            if key == 'link' and value:
                value = f'<{value}>'
            content = nodes.paragraph(text=value)
            row = nodes.row()
        ...
        tgroup += [nodes.colspec(colwidth=30), nodes.colspec(colwidth=70)]
        tbody = nodes.tbody()
        tbody.extend(rows)
        tgroup += tbody
        table = nodes.table()
